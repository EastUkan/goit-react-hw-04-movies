...
export default function MovieDetailsView() {
  ...
  useEffect(() => {
    setLoader(true);
    fetchMoviesDetailsViews(movieId)
      .then(movie => {
  ...
  return (
    <>
      {loader && <Loader />}
      <button type="button" onClick={goBack} className={s.goBack}>
        Назад
      </button>
      {movie ? (
        <div className={s.flex}>
          <img
            src={movie.poster_path ? `${URL_IMG}${movie.poster_path}` : NoImage}
            ...
            <div className={s.listGenres}>
              {movie.genres ? (
                movie.genres.map((genre, index) => (
                  <p key={index} className={s.listItem}>
                    {genre.name}
                  </p>
                ))
              ) : (
        ...
        <h2 className={s.noDetails}>
          No detail information for this film! {error}
        </h2>
      )}
    </>
  );
